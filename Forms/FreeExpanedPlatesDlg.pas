...
AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod GetGridSpacing(OveralSize:TFloatType):TFloatType \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.FGetActivePatch:TFreeDevelopedPatch \n');
  CloseFile (LOG);
   Result:=nil;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.FSetActivePatch(Val:TFreeDevelopedPatch) \n');
  CloseFile (LOG);
   Current:=ActivePatch;
   if Val<>Current then
   begin
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.FUpdateListBox \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ViewportRequestExtents(Sender: TObject;var Min, Max: T3DCoordinate) \n');
  CloseFile (LOG);
   if FPlates<>nil then
   begin
      N:=1;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.Execute(FreeShip:TFreeShip;Plates:TFasterList):boolean \n');
  CloseFile (LOG);
   FFreeship:=FreeShip;
   FPlates:=Plates;
   FUpdateListBox;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeCrosscurvesDialog.Execute(Freeship:TFreeship):Boolean \n');
  CloseFile (LOG);
   if FPlates<>nil then
   begin
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ViewportMouseMove(Sender: TObject;Shift: TShiftState; X, Y: Integer) \n');
  CloseFile (LOG);
...
AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ViewportMouseDown(Sender: TObject;Button: TMouseButton; Shift: TShiftState; X, Y: Integer) \n');
  CloseFile (LOG);
     FInitialPosition.X:=X;
   FInitialPosition.Y:=Y;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ListBoxClick(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ViewportMouseUp(Sender: TObject;Button: TMouseButton; Shift: TShiftState; X, Y: Integer) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.RotateCCW90Execute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.RotateCCW5Execute(Sender: TObject) \n');
  CloseFile (LOG);
   if ActivePatch<>nil then
   begin
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.RotateCW5Execute(Sender: TObject) \n');
  CloseFile (LOG);
   if ActivePatch<>nil then
   begin
      ActivePatch.Rotation:=ActivePatch.Rotation-5;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.RotateCW90Execute(Sender: TObject) \n');
  CloseFile (LOG);
   if ActivePatch<>nil then
   begin
      ActivePatch.Rotation:=ActivePatch.Rotation-90;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ZoomExtentsExecute(Sender: TObject) \n');
  CloseFile (LOG);
   Viewport.ZoomExtents;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.SaveBitmapExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ZoomInExecute(Sender: TObject) \n');
  CloseFile (LOG);
   Viewport.ZoomIn;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ZoomOutExecute(Sender: TObject) \n');
  CloseFile (LOG);
   Viewport.ZoomOut;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.RotateCCW1Execute(Sender: TObject) \n');
  CloseFile (LOG);
   if ActivePatch<>nil then
   begin
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.RotateCW1Execute(Sender: TObject) \n');
  CloseFile (LOG);
   if ActivePatch<>nil then
   begin
      ActivePatch.Rotation:=ActivePatch.Rotation-1;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ExportDXFExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowStationsExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowButtocksExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowWaterlinesExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowInteriorEdgesExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowFillColorExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ToolButton22Click(Sender: TObject) \n');
  CloseFile (LOG);
   Close;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowErrorEdgesExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowDiagonalsExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.PrintExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowDimensionsExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.Edit2KeyPress(Sender: TObject;var Key: Char) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.Edit2Exit(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.Edit3KeyPress(Sender: TObject;var Key: Char) \n');
  CloseFile (LOG);
   if (Key in [#8,'1'..'9','0',#13]) or (Key=DecimalSeparator) then else key:=#0;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.Edit3Exit(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.Edit1KeyPress(Sender: TObject;var Key: Char) \n');
  CloseFile (LOG);
   if (Key in [#8,'1'..'9','0','-',#13]) or (Key=DecimalSeparator) then else key:=#0;
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.Edit1Exit(Sender: TObject) \n');
  CloseFile (LOG);
   if ActivePatch<>nil then
   begin
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowPartNameExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.CheckBox1Click(Sender: TObject) \n');
  CloseFile (LOG);
   if ActivePatch<>nil then
   begin
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ShowSubmergedAreaExecute(Sender: TObject) \n');
  CloseFile (LOG);
...
begin
  AssignFile (LOG,'LogFile.txt');
  Append (LOG);
  Writeln (LOG,'Pozvan je metod TFreeExpanedplatesDialog.ExportTextFileExecute(Sender: TObject) \n');
  CloseFile (LOG);
   SaveDialog:=TSaveDialog.Create(Owner);
   SaveDialog.InitialDir:=FFreeship.Preferences.ExportDirectory;
   Str:=ChangeFileExt(ExtractFilename(FFreeship.FileName),'');
